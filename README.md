...
保存设置时**只有改动了地址 / 模式 / DNS / DHCP / IPv6 才会自动同步**到 RouterOS。
接口改名（`ether1/2` → `wan/lan`）这类改动表单比不出差异，所以「网络」卡片里提供了
**「同步网络配置并重启」**按钮，主动把当前配置重新写入客户机磁盘。

它会停止虚拟机 → 启动一次维护实例写盘 → 恢复原来的运行状态，期间客户端断网一两分钟。
...
默认是**直通**：Android 侧的 `ra6` 在客户端网段上广播运营商的 /64，客户端拿到的是
**真正的公网 IPv6 地址**，RouterOS 不参与 v6 转发。

> **`ra6` 必须打过补丁。** 它出厂写死 RA 生存期 45 秒，而 Android 15 起
> `accept_ra_min_lft=180` 会把生存期低于 180 秒的 RA 整条丢弃 —— 结果就是电脑有
> IPv6、手机全都没有。`patch-ra6.py` 把三个生存期常量改成 1800/7200/3600 秒；
> `build-package.sh` 打包时自动施加，`preflight` 会报 `ra6=patched` 还是 `ra6=short`。

另一种是让 **RouterOS 下发**（`IPV6_PASSTHROUGH=0`，仅网关模式）：LAN 用一段
ULA，出口做 NAT66。这样 v6 流量会经过 RouterOS 的防火墙，代价是客户端拿不到公网
地址。运营商只给一个 /64 且没有 PD，所以没法把公网段切给 LAN 用。

切回直通时有个坑：RouterOS 只在「接口上有 `advertise=yes` 的地址」时才广播，地址一删
它立刻静默、**不会发撤销**，而同步走的是离线维护引导（那一刻 RouterOS 不在网上）。
所以客户端会挂着失效的默认路由，最长 30 分钟才恢复。没有特别理由的话，用直通。

## 数据盘（容器 / App）

RouterOS 的容器不能跑在系统分区上，需要一块 `/disk` 下的盘，而 CHR 只带一块。
在「虚拟磁盘与备份」里勾选**挂载数据盘**、设好容量（1–64 GiB），插件会**在设备本地
生成**一个稀疏镜像挂给客户机 —— 不需要上传任何文件，标称容量也不会立刻占用存储。

挂载 / 卸载数据盘**必须重启虚拟机**（`virt` 机型的 `pcie.0` 不支持热插拔，实测报
`Bus 'pcie.0' does not support hotplugging`）。

盘挂上之后，在 RouterOS 里格式化一次：
...
- 容器功能本身还需要 MikroTik 的 `container` 扩展包（.npk），本项目不含
- **备份只包含系统盘**，数据盘不在里面；删除数据盘不可恢复
